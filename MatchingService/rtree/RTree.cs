...
namespace MatchingService.rtree
{
    ...
    public class RTree<T>
    {
        ...
        List<ElementDistance<T>> nearestIds = new List<ElementDistance<T>>();
        ...
        private void init()
        {
            ...
            if (maxNodeEntries < 2)
            {
                Console.WriteLine("Invalid MaxNodeEntries = " + maxNodeEntries + " Resetting to default value of " + DEFAULT_MAX_NODE_ENTRIES);
            ...
            {
                Console.WriteLine("MinNodeEntries must be between 1 and MaxNodeEntries / 2");
            ...
            Console.WriteLine("init() " + " MaxNodeEntries = " + maxNodeEntries + ", MinNodeEntries = " + minNodeEntries);
        }
        ...
        private bool delete(Rectangle r, int id)
        {
            ...
            while (foundIndex == -1 && parents.Count > 0)
            {
                ...
                if (!n.isLeaf())
                {
                    Console.WriteLine("searching Node<T> " + n.nodeId + ", from index " + startIndex);
                    bool contains = false;
        ...
        public ElementDistance<T> Nearest(Point p, double furthestDistance)
        {
            Node<T> rootNode = getNode(rootNodeId);
            nearest(p, rootNode, furthestDistance);
            if (nearestIds.Count > 0)
            {
                ElementDistance<T> retval = new ElementDistance<T>(nearestIds[0].Id, IdsToItems[nearestIds[0].Id],
                    nearestIds[0].Distance);
                nearestIds.Clear();
                return retval;
            }
            return null;
        ...
        public List<ElementDistance<T>> KNN(Point p, double furthestDistance)
        {
            Node<T> rootNode = getNode(rootNodeId);
            List<ElementDistance<T>> retval = new List<ElementDistance<T>>();
            Knearest(p, rootNode, furthestDistance);
            foreach (ElementDistance<T> element in nearestIds)
            {
                retval.Add(new ElementDistance<T>(element.Id, IdsToItems[element.Id], element.Distance));
            }
            nearestIds.Clear();
            return retval;
        }

        private void Knearest(Point p, Node<T> n, double nearestDistance)
        {
            for (int i = 0; i < n.entryCount; i++)
            {
                ...
                { // for leaves, the distance is an actual nearest distance 
                    ...
                    {
                        nearestIds.Add(new ElementDistance<T>(n.ids[i], tempDistance));
                    }
                }
                else
                { // for index nodes, only go into them if they potentially could have
                    ...
                    if (tempDistance <= nearestDistance)
                    {
                        ...
                        Knearest(p, getNode(n.ids[i]), nearestDistance);
                    }
                }
            }
        }
        ...
        */
        ...
        private Node<T> splitNode(Node<T> n, Rectangle newRect, int newId)
        {
            ...
            if (INTERNAL_CONSISTENCY_CHECKING)
            {
                ...
                {
                    Console.WriteLine("Error: splitNode old Node<T> MBR wrong");
                }

                if (!newNode.mbr.Equals(calculateMBR(newNode)))
                {
                    Console.WriteLine("Error: splitNode new Node<T> MBR wrong");
                }
            }
        ...
        private void pickSeeds(Node<T> n, Rectangle newRect, int newId, Node<T> newNode)
        {
            ...
            for (int d = 0; d < Rectangle.DIMENSIONS; d++)
            {
                ...
                for (int i = 0; i < n.entryCount; i++)
                {
                    ...
                    {
                        Console.WriteLine("Invalid normalized separation");
                    }
        ...
        private int pickNext(Node<T> n, Node<T> newNode)
        {
            ...
            for (int i = 0; i < maxNodeEntries; i++)
            {
                if (entryStatus[i] == ENTRY_STATUS_UNASSIGNED)
                {
                    ...
                    if (n.entries[i] == null)
                    {
                        Console.WriteLine("Error: Node<T> " + n.nodeId + ", entry " + i + " is null");
                    }
        ...
        private Node<T> chooseNode(Rectangle r, int level)
        {
            ...
            while (true)
            {
                if (n == null)
                {
                    Console.WriteLine("Could not get root Node<T> (" + rootNodeId + ")");
                }
        ...
        private Node<T> adjustTree(Node<T> n, Node<T> nn)
        {
            ...
            while (n.level != treeHeight)
            {
                ...
                {
                    Console.WriteLine("Error: entry " + entry + " in Node<T> " +
        ...
        private void checkConsistency(int nodeId, int expectedLevel, Rectangle expectedMBR)
        {
            ...
            if (n == null)
            {
                Console.WriteLine("Error: Could not read Node<T> " + nodeId);
            }

            if (n.level != expectedLevel)
            {
                Console.WriteLine("Error: Node<T> " + nodeId + ", expected level " + expectedLevel + ", actual level " + n.level);
            }
            ...
            {
                Console.WriteLine("Error: Node<T> " + nodeId + ", calculated MBR does not equal stored MBR");
            }

            if (expectedMBR != null && !n.mbr.Equals(expectedMBR))
            {
                Console.WriteLine("Error: Node<T> " + nodeId + ", expected MBR (from parent) does not equal stored MBR");
            }
            ...
            {
                Console.WriteLine("Error: Node<T> " + nodeId + " MBR using same rectangle object as parent's entry");
            }
            ...
            for (int i = 0; i < n.entryCount; i++)
            {
                if (n.entries[i] == null)
                {
                    Console.WriteLine("Error: Node<T> " + nodeId + ", Entry " + i + " is null");
                }
