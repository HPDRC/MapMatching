...
namespace MatchingService.Astar
{
    ...
    public class Point2D
    {
        ...
        public static Point2D ProjectOnLine(Point2D pt, Point2D p1, Point2D p2, out NearestPostion position)
        {
            ...
            if (distance < 0)
            {
                position = NearestPostion.Start;
                return p1;
            }
            if (distance > 1)
            {
                position = NearestPostion.End;
                return p2;
            }
            position = NearestPostion.Middle;
            return p1 + vLine * distance;
        }

        public static Point2D ProjectOnLine(Point2D pt, Arc arc, out NearestPostion position)
        {
